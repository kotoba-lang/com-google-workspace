(ns google-workspace.main
  "Kotodama WASM entrypoint for the Google-workspace clean-room actor (L4) — Clojure port.
  ...
  facts (`google_workspace.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "google_workspace")
(def tier "L4")
...
(def entity-specs
  [{:entity "Workspace" :plural "workspaces" :id-prefix "googlewo_wor"
    :fields [:name :ownerId :plan]                :required [:name :plan]
    :coerce {}                       :refs {}}
   {:entity "Document"  :plural "documents" :id-prefix "googlewo_doc"
    :fields [:workspaceId :title :contentRef :ownerId] :required [:title]
    :coerce {}                       :refs {:workspaceId "Workspace"}}
   {:entity "Folder"    :plural "folders"    :id-prefix "googlewo_fol"
    :fields [:workspaceId :name :parentId]         :required [:name]
    :coerce {}                       :refs {:workspaceId "Workspace"}}
   {:entity "Comment"   :plural "comments"   :id-prefix "googlewo_com"
    :fields [:documentId :authorId :body]          :required [:body]
    :coerce {}                       :refs {:documentId "Document"}}
   {:entity "Permission" :plural "permissions" :id-prefix "googlewo_per"
    :fields [:resourceId :principalId :role]       :required [:role]
    :coerce {}                       :refs {}}
   {:entity "User"      :plural "users"      :id-prefix "googlewo_use"
...
(defn emit-facts
  "EAVT facts for one record: {\"google_workspace.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "google-workspace-compat" :tier tier :entities entities} 200])
...
(defn start! [] :google-workspace-compat/ready)
